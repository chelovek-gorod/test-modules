...
const textCursor = new Text('CURSOR / | | \\ x: 0; y: 0', Math.floor(screen.width / 2), screen.height - 12, textCursorOptions);
const cursorState = {
    x: 0, 
    y: 0,
    isClick: false,
...
function testCursorUpdate() {
    ...
    if (CURSOR.isRightClick !== cursorState.isRightClick) {cursorState.isRightClick = CURSOR.isRightClick; isChanged = true;}
    if (CURSOR.isWheelClick !== cursorState.isWheelClick) {cursorState.isWheelClick = CURSOR.isWheelClick; isChanged = true;}
    if (CURSOR.isWheelUp !== cursorState.isWheelUp) {cursorState.isWheelUp = CURSOR.isWheelUp; isChanged = true;}
    if (CURSOR.isWheelDown !== cursorState.isWheelDown) {cursorState.isWheelDown = CURSOR.isWheelDown; isChanged = true;}
    if (isChanged) {
        let left = CURSOR.isClick ? '*' : ' ';
        let right = CURSOR.isClick ? '*' : ' ';
        let wheel = '';
        if (CURSOR.isWheelClick) wheel = '*';
        else if (CURSOR.isWheelUp) wheel = '+';
        else if (CURSOR.isWheelDown) wheel = '-';
        textCursor.render(`CURSOR /${left}|${wheel}|${right}\\ x: ${CURSOR.x}; y: ${CURSOR.y}`);
    }
